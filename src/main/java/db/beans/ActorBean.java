...
public class ActorBean {
    ...
    public static ActorBean createNewActor(ActorBean actorbean) throws SQLException {
        String columns = getAllColumns(actorbean);
        String values = getValuesPlaceholder(actorbean);
        String insertQuery = "INSERT INTO actor (" + columns + ") VALUES (" + values + ")";
        String retrieveQuery = "SELECT * FROM actor WHERE first_name = " + actorbean.getFirst_name() + " AND last_name = "
                + actorbean.getLast_name();
        return DBConnection.insertAndRetrieveBean(insertQuery, retrieveQuery, ActorBean.class,
                actorbean.getFirst_name(),actorbean.getLast_name());
